...
coro_actor_ctx::coro_actor_ctx(asio::io_context& io_ctx, std::shared_ptr<asio::local::stream_protocol::socket> unix_socket,
    std::unordered_map<std::string, coro_actor_ctx::func_t> service_map) 
    : io_ctx_(io_ctx), unix_socket_(unix_socket), pid_(getpid()), seq_(0),
      service_map_(std::move(service_map)), msg_notify_timer_(io_ctx_), service_notify_timer_(io_ctx_),
      response_notify_tiemr_(io_ctx_), is_waiting(false)
{
    this->msg_notify_timer_.expires_at(std::chrono::steady_clock::time_point::max());
    this->service_notify_timer_.expires_at(std::chrono::steady_clock::time_point::max());
    this->response_notify_tiemr_.expires_at(std::chrono::steady_clock::time_point::max());

    asio::co_spawn(this->io_ctx_, handle_message(), asio::detached);
}

asio::io_context& coro_actor_ctx::io_contex() {
    return this->io_ctx_;
}
...
std::string coro_actor_ctx::get_error_msg() const {
...
asio::awaitable<bool> coro_actor_ctx::send(const std::string& service_name, const coro_actor_pack_t* request_pack, coro_actor_pack_t* response_pack) {
    /* 检查服务是否在本 actor 进程提供的列表中 */
    if (this->contains(service_name)) {
        /* 在上下文设置错误信息, 记录错误日志 */
        this->set_error_msg("This service is already supported by this process");
        co_return false;
    }
    ...
    co_await asio::async_write(*(this->unix_socket_), write_buf, asio::redirect_error(asio::use_awaitable, ec));
    if (ec) {
    ...
    /* 等待响应 */
    co_await this->response_notify_tiemr_.async_wait(asio::redirect_error(asio::use_awaitable, ec));
...
asio::awaitable<void> coro_actor_ctx::handle_message() {
    ...
    while (!this->io_ctx_.stopped()) {
        if (this->msg_queue_.empty()) {
            ...
            /* 调用成功则发送响应消息 */
            if (msg.socket) {
                length = asio::detail::socket_ops::host_to_network_long(resp_pack.value.length());

                std::array<asio::const_buffer, 3> write_buf = {
                    { asio::buffer(&resp_pack.type, 1), asio::buffer(&length, 4),
                      asio::buffer(resp_pack.value.data(), resp_pack.value.length()) }
                };

                co_await asio::async_write(*msg.socket, write_buf, asio::redirect_error(asio::use_awaitable, ec));
                ...
                co_await asio::async_write(*msg.unix_socket, write_buf, asio::redirect_error(asio::use_awaitable, ec));
            }
        }
    }
