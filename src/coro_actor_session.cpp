...
asio::awaitable<void> coro_actor_session::handle_remote_pack() {
    ...
    while (this->socket_->is_open()) {
        ...
        service_length =
            asio::detail::socket_ops::network_to_host_short(service_length);
        ...
        data_length =
            asio::detail::socket_ops::network_to_host_long(data_length);
        ...
        msg.pack =
            coro_actor_pack_t{.type = type, .value = std::move(request_data)};
