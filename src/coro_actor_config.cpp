...
/* 对于每个 Actor 节点, 需要解析节点名、模块文件名、描述具体服务名的配置文件, 可选的监听地址, 表示服务对外开放 */
/* 注意不同 Actor 节点不能配置相同的监听地址且所有 Actor 提供的服务不能存在重复即服务名必须全局唯一 */
      ...
      try {
            ...
            for (const auto& actor_detail : root["actor_config"]) {
                  ...
                  if (check_actor_name.contains(cfg.name)) {
                        throw std::runtime_error(fmt::format("Cannot set the same 'actor_name' : [{}]", cfg.name));
                  } else {
                        check_actor_name.emplace(cfg.name);
                  }

                  YAML::Node services = YAML::LoadFile(actor_detail["actor_service"].as<std::string>());
                  for (const auto& service : services["services"]) {
                        auto service_name = service.as<std::string>();
                        if (check_service.contains(service_name)) {
                              throw std::runtime_error(fmt::format("Cannot set the same 'service_name' : [{}]", service_name));
                        } else {
            ...
            SPDLOG_ERROR("failed to parse config file: [{}], error info: {}", file, e.what());
            return false;
      }
